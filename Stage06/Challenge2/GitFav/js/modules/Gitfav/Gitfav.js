...
export class GitfavData {
  ...
  async addUserDataInTable(user) {
    try {
      ...
      const userExists = this.usersData.find(
        (user) => userData.login === user.login,
      );
  ...
  removeUserDataFromTable(user) {
    ...
    const filteredData = this.usersData.filter(
...
`;
  ...
  setUserRowInTable() {
    ...
    this.usersData.forEach((data) => {
      const dataRow = this.createUserRow(
        data.login,
        data.name,
        data.public_repos,
        data.followers,
      ...
      deleteButon.onclick = () => {
        const deleteUser = confirm(
          `Deseja desfavoritar o usuário "${data.login}"?`,
        );
